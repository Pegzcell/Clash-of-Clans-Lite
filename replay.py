...
game.filename = input("Enter name of file: ")

def openit():
    f = open("replays/"  +game.filename, "r")
    game.inputs = ((f.read()).strip("\n")).split("\n")
    game.playerType = game.inputs.pop(0)
    print(len(game.inputs))
    f.close()
